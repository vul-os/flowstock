...
function makeHttpDriver() {
  ...
  return {
    ...
    putRow: (tbl, id, data) =>
      req("POST", `/api/rows/${tbl}`, { id: id || "", data }),
    ...
    savePurchaseOrder: (payload) =>
      req("POST", "/api/purchase-orders/save", payload),
    ...
    newSyncSecret: () =>
      req("GET", "/api/sync/secret/new").then((r) => r.secret),
    ...
    syncNow: (peerId) =>
      req("POST", "/api/sync/now", { peer_id: peerId || "" }),
  ...
  return {
    ...
    putRow: async (tbl, id, data) => {
      if (tbl === "stock_movements")
        throw new Error("stock movements are immutable");
    ...
    saveOrder: async ({ order, items, services }) => {
      ...
      if ((existing ? existing.status : "draft") === "draft") {
        ...
        if (services)
          replaceChildren("order_services", "order_id", id, services);
      }
      changed();
      return { ...row };
    },
    setOrderStatus: async (orderId, status) => {
      ...
      const sales = live("stock_movements").filter(
        (m) =>
          m.ref_kind === "order" && m.ref_id === orderId && m.kind === "sale",
      );
    ...
    receivePurchaseOrder: async (poId, receipts) => {
      ...
      const items = live("purchase_order_items").filter(
        (i) => i.purchase_order_id === poId,
      );
      ...
      const stockable = items.filter(
        (i) => (i.item_type || "product") === "product",
      );
      const all =
        stockable.length > 0 &&
        stockable.every(
          (i) => (i.received_quantity || 0) + 1e-9 >= (i.quantity || 0),
        );
    ...
    setSyncSettings: async (cfg) => {
      const d = load();
      Object.assign(d.sync, cfg, {
        bind_addr: cfg.bindAddr ?? d.sync.bind_addr,
      });
      changed();
      return { ...d.sync, listening: d.sync.listen, node_id: "DEMO-NODE" };
    },
